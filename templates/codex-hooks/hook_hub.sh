...
# Central hook adapter hub for __RUNNER__.
...
# Resolve symlinks so a project-local adapter can identify the metadata root of
# the central hub it actually targets, even without wrapper-provided variables.
...
# Explicit overrides remain authoritative. Direct runner/GUI hook calls derive
# the storage root from <storage>/.agent-context-engine/hooks/<runner>/.
...
# Resolve the CLI script by convention.
