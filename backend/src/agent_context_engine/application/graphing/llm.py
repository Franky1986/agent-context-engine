...
def llm_graph_run(
    ...
    *,
    ...
    if runner == "codex":
        ...
        proc = subprocess.run(command, input=prompt, text=True, capture_output=True, timeout=timeout, cwd=str(ROOT), env={**os.environ, "AGENT_MEMORY_DREAM": "1", "AGENT_MEMORY_ROOT": str(ROOT)})
        tool_event_detected = False
        raw_text = proc.stdout
        response_path.write_text(raw_text, encoding="utf-8")
    elif runner == "cursor":
        command = cursor_graph_command(resolved_model)
        proc = subprocess.run(command + [prompt], text=True, capture_output=True, timeout=timeout, cwd=str(ROOT), env={**os.environ, "AGENT_MEMORY_DREAM": "1", "AGENT_MEMORY_ROOT": str(ROOT)})
        tool_event_detected = False
